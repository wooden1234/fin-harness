...
SKILL_NAME = "hithink-fund-selector"
SKILL_VERSION = "1.0.0"
...
class FundSelectorAPIError(Exception):
    """API 错误异常类"""
...
def query_fund(
    ...
    Raises:
        FundSelectorAPIError: API 调用失败时抛出
    """
    ...
    try:
        ...
        raise FundSelectorAPIError(f"网络错误: {e.reason}")


def parse_args():
    """解析命令行参数"""
    parser = argparse.ArgumentParser(
        description="同花顺智能选基金 - 基金数据查询工具",
...
def main():
        ...
        if isinstance(result, dict) and "text_response" not in result:
            ...
            if has_more:
                output["pagination_tip"] = (
                    f"共查到 {code_count} 只基金，当前返回第 {args.page} 页的 {len(datas)} 条。"
    ...
    except FundSelectorAPIError as e:
