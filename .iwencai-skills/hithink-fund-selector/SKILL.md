...
| `X-Claw-Skill-Id` | `hithink-fund-selector`（与 skill name 一致） |
...
headers = {
    ...
    "X-Claw-Skill-Id": "hithink-fund-selector",
...
**注意：** 默认返回 10 条数据，但符合条件的基金总数可能更多，需关注 `code_count` 字段并通过分页获取全部数据。
...
```python
for item in datas:
    fund_code = item.get("基金代码")       # 如 "000001.OF"
    fund_name = item.get("基金简称")       # 如 "华夏成长混合"
    # 其他字段根据查询类型而定
```
...
| code_count | INT | 符合查询条件的总基金数量（注意：可能大于当前返回的 datas 条数） |
...
python3 scripts/cli.py --query "股票型基金有哪些？" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "股票型基金有哪些？" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-fund-selector" \
...
} -Body '{"query":"股票型基金","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
