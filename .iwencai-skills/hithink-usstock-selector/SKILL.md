...
| `X-Claw-Skill-Id` | `hithink-usstock-selector`（与 skill name 一致） |
...
**注意：** 默认返回 10 条数据，但符合条件的美股总数可能更多，需关注 `code_count` 字段并通过分页获取全部数据。
...
```python
for item in datas:
    stock_name = item.get("股票名称")       # 如 "苹果"
    # 其他字段根据查询类型而定，如涨跌幅、市盈率、评级等
```
...
| code_count | INT | 符合查询条件的总美股数量（注意：可能大于当前返回的 datas 条数） |
...
python3 scripts/cli.py --query "美股科技股有哪些？" --call-type "retry"

# 指定超时时间（复杂查询可适当增加）
python3 scripts/cli.py --query "美股科技股有哪些？" --timeout 60
```
...
curl -X POST "https://openapi.iwencai.com/v1/query2data" \
  ...
  -H "X-Claw-Skill-Id: hithink-usstock-selector" \
...
} -Body '{"query":"评级买入的美股","page":"1","limit":"10","is_cache":"1","expand_index":"true"}'
```
