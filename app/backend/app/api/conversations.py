...
@router.get("/export")
async def export_user_data(current_user: AuthUser = Depends(get_current_user)):
    """导出当前用户的会话、消息和 Agent 运行数据。"""
    return await PrivacyService.export_user_data(current_user.id)


@router.delete("/data")
async def request_user_data_delete(current_user: AuthUser = Depends(get_current_user)):
    """请求删除当前用户数据，checkpoint 通过 outbox 异步清理。"""
    count = await PrivacyService.request_user_data_delete(
...
@router.post("")
async def create_conversation(current_user: AuthUser = Depends(get_current_user)):
...
async def get_conversation_messages(
    ...
    try:
        return await ConversationService.get_conversation_messages(
            conversation_id, current_user.id, current_user.tenant_id
        )
